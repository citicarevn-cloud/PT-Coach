export type MealType = "BREAKFAST" | "LUNCH" | "PRE_WORKOUT" | "DINNER";
...
export interface MenuMeal {
  ...
  portionScale: number;
  items: FoodItem[];
  totals: Omit<FoodItem, "name" | "amount">;
}

export const foodDatabase: Record<MealType, FoodOption[]> = {
  BREAKFAST: [
    {
...
const mealPlan: Array<{ type: MealType; label: string; ratio: number }> = [
...
export function generateVietnameseMenu(
  targetCaloriesKcal: number,
  calciumFocus: boolean,
...
function pickRandom<T>(options: T[], random: () => number): T {
